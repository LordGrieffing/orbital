...
import numpy as np
import parallelBodies as body
import random as rd
import concurrent.futures

# Function to pass to executer
def compute_Vupdate(position, velocity, mass, ObjectsInfo, dt):
    temp_obj = body.massObject(position, velocity, mass, 1)

    return temp_obj.kUpdate(ObjectsInfo, dt)
...
def main():
    ...
    for i in range(num_steps):
        ...
        # get the stripped down object data
        objectsData = [(object.position, object.velocity, object.mass) for object in objects]

        # Calculate k constants with parallel processes
        with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = []

            for pos, vel, mass in objectsData:
                futures.append(executor.submit(compute_Vupdate, pos, vel, mass, objectsData, dt))

            kValues = [f.result() for f in futures]


        # Update position and velocity of objects
        object1.posVelUpdate(kValues[0][0], kValues[0][1], dt)
        object2.posVelUpdate(kValues[1][0], kValues[1][1], dt)
        object3.posVelUpdate(kValues[2][0], kValues[2][1], dt)
